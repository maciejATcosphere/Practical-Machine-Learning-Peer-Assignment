...
We read in **training.csv** file that will serve as a basis of our analysis. File **testing.csv** would be use to produce prediction for submission.
```{r}
...
Furthermore one can take a look at the original data set and just to measure how well the **classe** variable is spread among sample data, so visually confirm that it's faily equally distributed.
...
As one can the accuracy of the fitted model on the training set is inredible high around: 0.99 which on the first could suggest some serious overfitting that takes place during the training phase but as we show later testing data (out of sample error) prove that actually the model is incredibly successful and it's accuracy is not due to overfitting. 
...
Courisily enough one could try to invesitgate deeply what's the reason behind such an incredible performance by looking at the variable importance
```{r}
varImp(modFit)
```
...
We measure a performance of our predictive model on the hold-out set (part of the initial data). As one can see the final accuracy on the testing set is very high and it's around: 0.99. 
```{r}
